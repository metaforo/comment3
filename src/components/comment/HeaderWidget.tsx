...
export default function HeaderWidget(
    ...
    if (Global.isDebug && thread) {
        // @ts-ignore
        const link = `${process.env.REACT_APP_API_HOST.replace('/api', '/')}g/${commentWidgetState.siteName}/thread/${thread.id}`;
        debugBtn = (<a href={link}
                       style={{
                           marginLeft: '12px',
                           fontSize: '10px',
                       }}
